...
class IMButtonTests: XCTestCase {
    ...
    func testIMButtonCreate() {
        ...
        XCTAssertEqual(button.backgroundColor, backgroundColor,
                       "IMButton backgroundColor is \(String(describing: button.backgroundColor)) expected: \(backgroundColor)")
        let expectedCornerRadius: CGFloat = 10.0
        XCTAssertEqual(button.layer.cornerRadius, expectedCornerRadius,
                       "IMButton corner radius is \(button.layer.cornerRadius) expected: \(expectedCornerRadius)")
        let titleColor = UIColor.white
        XCTAssertEqual(button.titleColor(for: .normal), titleColor,
                       "IMButton title color was \(String(describing: button.titleColor(for: .normal))) expected: \(titleColor)")
        let expectedFont = UIFont.preferredFont(forTextStyle: .headline)
        XCTAssertEqual(button.titleLabel?.font, expectedFont,
                       "IMButton title font was \(String(describing: button.titleLabel?.font)) expected: \(expectedFont)")
